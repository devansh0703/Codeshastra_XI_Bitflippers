...
# ---------------------------
# CONTRACT ABIs: Load actual ABIs from JSON files
# Files: Voting.json, VoterNFT.json, Verifier.json (placed in the same directory as main.py)
...
# (Verifier is used on-chain; backend doesn't directly call its functions)
...
# ---------------------------
# 'users' table: identity_number, name, email, password_hash, role, image_base64, created_at
...
# 'votes' table: vote_id, voter_identity, candidate_id, election_id, voter_geolocation, blockchain_tx_hash, created_at
...
# 'disputes' table: id, vote_id, reason, status, created_at
...
# Registration Endpoint: Maps "full_name" to "name" to match DB schema
@app.post("/register")
def register_user(req: RegisterRequest):
    ...
    # Map 'full_name' from request to 'name' for the database.
    user_data = {
...
# Login Endpoint with Face Verification
@app.post("/login")
def login_user(req: LoginRequest):
    if len(req.identity_number) != 12 or not req.identity_number.isdigit():
        raise HTTPException(status_code=400, detail="Invalid Aadhaar number")
    response = supabase.table("users").select("*").eq("identity_number", req.identity_number).execute()
...
def cast_vote(vote_req: VoteRequest):
    ...
    certificate_data = {
        ...
        "voter_identity": vote_req.voter_identity,
        "election_id": vote_req.election_id,
        "nft_token_id": "TBD",  # Extract token ID from nft_receipt events if available
